...
server <- function(input, output, session) {
  output$map <- renderLeaflet({
    ...
    if (nrow(filtered_df) < 5) {stop("Not enough unique dropoff locations from this origin point. Please select another starting area location.")}
    
     #filter second projection
    proj2 <- subset(proj, proj$zone %in% paste(input$location))
